...
static int lsocket(int domain, int type, int protocol)
{
    // only support UDP
    if (type != SOCK_DGRAM)
        return -1;
...
static int lbind(int sockfd, const struct sockaddr *addr, socklen_t addrlen)
{
...
static ssize_t lrecvfrom(int sockfd, void *buf, size_t len, int flags,
...
static ssize_t lsendto(int sockfd, const void *buf, size_t len, int flags,
