...
{
    int sockfd = lsocket(AF_INET, SOCK_DGRAM, 0);
    ...
    lbind(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr));
    ...
    for (;;) {
        ...
        if ( (recvd = lrecvfrom(sockfd, buf, sizeof(buf), 0, (struct sockaddr *)&cliaddr, &addrlen)) < 0)
            continue;
        else
            printf("recv from %s:%d, data: %s\n", inet_ntoa(cliaddr.sin_addr), ntohs(cliaddr.sin_port), buf);

        lsendto(sockfd, buf, recvd, 0, (struct sockaddr *)&cliaddr, sizeof(cliaddr));
    }
