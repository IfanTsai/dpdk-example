...
{
    // request arp first, then callback by timer
    arp_request_timer_cb(NULL, mpool);
...
static int lcore_process_pkt(void *arg)
{
    ...
    for (;;) {
        ...
        for (unsigned int i = 0; i < nr_recvd; i++) {
            ...
            case RTE_ETHER_TYPE_IPV4: {
                ...
                case IPPROTO_UDP:
                    if (process_udp_pkt(mbufs[i]) < 0)
                        goto mbuf_free;
                    continue;

                case IPPROTO_TCP:
                    if (process_tcp_pkt(mbufs[i]) < 0)
                        goto mbuf_free;
                    continue;
